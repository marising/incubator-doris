...
public class CacheAnalyzer {
    ...
    private CacheMode innerCheckCacheMode(long now) {
        ...
        for (int i = 1; i < tblTimeList.size(); i++) {
            if ((now - tblTimeList.get(i).latestTime) < Config.last_version_interval_second * 1000) {
    ...
    public CacheBeProxy.FetchCacheResult getCacheData() {
        ...
        if (cacheMode == CacheMode.None) {
            LOG.info("check cache model {}, queryid {}", cacheMode, DebugUtil.printId(queryId));
            return cacheResult;
        }
        ...
        if (status.ok() && cacheResult != null) {
            LOG.info("hit cache, model {}, queryid {}, all count {}, value count {}, row count {}, data size {}",
            ...
            LOG.info("miss cache, model {}, queryid {}, code {}, msg {}", cacheMode,
